...
compilation_database_folder = "/src/"
...
def Settings(**kwargs):
    ...
    if language == "cfamily":
        ...
        def fixYCMBug(flag):
            """Fix flags like this: '-isystem/src/build/=_deps/jitify-src'.
            This is caused by the 'include_paths_relative_to_dir' key: YCM
            incorrectly inserts the absolute path."""
            if "-isystem=" in flag:
                return flag.split("=")
            return [flag]

        final_flags = [
            flag for compoundFlag in final_flags for flag in fixYCMBug(compoundFlag)
        ]

        # add system local std-lib
        final_flags += ["-isystem", "/usr/include/c++/11/"]
        final_flags += ["-isystem", "/usr/include/x86_64-linux-gnu/c++/11/"]
